<template>
  <div class="movie-list-container">
    <h1>Рекомендации</h1>
    <div v-if="hasPreferences === null" class="loading-message">Загрузка...</div>
    <div v-else-if="hasPreferences">
      <div class="filters">
      ...
      <p v-else class="no-movies">Нет рекомендованных фильмов, соответствующих фильтрам.</p>
    ...
    <div v-else class="no-preferences">
      <p>Вы ещё не настроили предпочтения.</p>
      <router-link to="/profile" class="setup-button">Настроить предпочтения</router-link>
    </div>
...
export default {
  ...
  setup() {
    ...
    const checkPreferences = async () => {
      ...
      try {
        ...
        if (err.response?.status === 404) {
          ...
          error.value = 'Ошибка проверки предпочтений.';
          console.error(err);
        }
      }
    };
    ...
    const fetchRecommendations = async (page) => {
      if (!checkAuth() || !hasPreferences.value) return;
      try {
        const userId = store.state.user.id;
        const response = await api.getMoviesByPreferences(userId, page, pageSize);
        movies.value = response.data.content;
        currentPage.value = response.data.page?.number || page;
        totalPages.value = response.data.page?.totalPages || response.data.totalPages || 1;
        error.value = '';
      } catch (err) {
        error.value = 'Ошибка загрузки рекомендаций.';
        console.error(err);
      }
    };
    ...
    const filteredMovies = computed(() => {
      ...
      if (yearFrom.value || yearTo.value) {
        filtered = filtered.filter((movie) => {
          const year = parseInt(formatYear(movie.releaseDate));
    ...
    onMounted(async () => {
      ...
      if (hasPreferences.value) {
        await fetchRecommendations(currentPage.value);
      }
    });
